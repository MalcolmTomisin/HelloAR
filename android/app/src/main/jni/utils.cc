    ...
    namespace utils
    {
        ...
        static GLuint LoadShader(GLenum shader_type, const char *shader_source)
        {
            ...
            if (!compiled)
            {
                ...
                LOGE("hello_ar::utils::Could not compile shader %d:\n%s\n", shader_type,
                     buf);
                free(buf);
                             ...
                             const std::map<std::string, int> &define_values_map)
        {
            ...
            if (program)
            {
                ...
                CheckGlError("hello_ar::utils::glAttachShader");
                ...
                if (link_status != GL_TRUE)
                {
                    ...
                    if (buf_length)
                    {
                        ...
                        if (buf)
                        {
                            ...
                            LOGE("hello_ar::utils::Could not link program:\n%s\n", buf);
                            free(buf);
                        }
                    }
        ...
        bool LoadPngFromAssetManager(int target, const char *path)
        {
            ...
            } jniIds = [env]() -> JNIData
            {
                ...
                LOGE("hello_ar::utils::Could not find Java helper class %s",
                     kHelperClassName);
                return {};
            }();
